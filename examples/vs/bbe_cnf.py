...
class RobustBlockVSModel(ParametricModelSpace):
    def __init__(self,rj_only=False,is_naive=False):
        random_variables = {}
        self.nblocks = 3
        ...
        if rj_only:
            proposal = RJZGlobalRobustBlockVSProposalSaturated(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep,is_naive=is_naive)
        else:
            proposal = SystematicChoiceProposal([RJZGlobalRobustBlockVSProposalSaturated(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep,is_naive=is_naive),mep])
...
#shelffilename='./SMC1_RobustBlockVS_mp_2block_saturated_shelve.out'
...
if sys.argv[1]=="reload":
    ...
    if True:
        mymodel = RobustBlockVSModel(rj_only=True)
        ...
        with open("results_robustvs_d4_2block_cnf_N{}.txt".format(NPARTICLES), "a") as file_object:
