...
class RobustBlockVSModel(ParametricModelSpace):
    def __init__(self,k=None,rj_only=False,is_naive=False):
        ...
        for i in range(self.nblocks):
            #random_variables[blocknames[i]]=TransDimensionalBlock({betanames[i]:NormalRV()},nblocks_name=gammanames[i],minimum_blocks=0,maximum_blocks=1)
        ...
        if k is None:
            if rj_only:
                #proposal = RJZGlobalRobustBlockVSProposal(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep,is_naive=is_naive)
                proposal = RJZGlobalBlockVSProposalIndiv(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep,affine=False)
            else:
                #proposal = SystematicChoiceProposal([RJZGlobalRobustBlockVSProposal(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep,is_naive=is_naive),mep])
                proposal = SystematicChoiceProposal([RJZGlobalBlockVSProposalIndiv(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep,affine=False),mep])
        else:
            proposal = mep
        #proposal = SystematicChoiceProposal([RJZGlobalRobustBlockVSProposal(self.blocksizes,self.blocknames,self.gammanames,self.betanames,mep),mep])
...
if sys.argv[1]=="reload":
    ...
    if True:
        ...
        with open("results_robustvs_d4_2block_inf_N{}.txt".format(NPARTICLES), "a") as file_object:
