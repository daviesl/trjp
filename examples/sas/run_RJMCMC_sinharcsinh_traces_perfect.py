    ...
    def calibratemmmpd(self,mmmpd,size,t):
        ...
        if True:
            for mk in mks:
                ...
                #self.flows[mk] = MaskedAffineFlow.factory(X,bnorm,bmmt,ls,fn)
...
if len(sys.argv) < 2 or sys.argv[1]=="run":
    ...
    # save it
    np.save('{}perfect_sas_rjmcmc_theta_run{}.npy'.format(pfx,run_no),final_theta)
    np.save('{}perfect_sas_rjmcmc_ptheta_run{}.npy'.format(pfx,run_no),prop_theta)
    np.save('{}perfect_sas_rjmcmc_ar_run{}.npy'.format(pfx,run_no),prop_theta)
