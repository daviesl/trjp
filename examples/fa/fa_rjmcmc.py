...
if __name__ == "__main__":
    ...
    #NP_list = [2000] # TODO use this list via arg
    ...
    if sys.argv[3]=='run':
        ...
        #pfx='/media/laurence/DATA/fa_simres/posteriors/'
        pfx='./'
        mk_theta_train = {}
        mk_theta_test = {}
        for k in range(1,3):
            mk_theta_train[k] = np.load('{}FA_pyMC_k{}_N{}_run{}_{}.npy'.format(pfx,k,NPARTICLES,run_no,'train'))
            mk_theta_test[k] = np.load('{}FA_pyMC_k{}_N{}_run{}_{}.npy'.format(pfx,k,NPARTICLES,run_no,'test'))
    ...
    rjmcmc = RJMCMC(fa_k1k2_model, calibrate_draws=train_theta)
    #for i in range(100):
    print("Run ",run_no)
    theta, prop_theta, llh, log_prior, ar = rjmcmc.run(200000,test_theta[np.random.choice(test_theta.shape[0],size=1)])
    np.save("results_FA_theta_{}_{}_{}_0b.npy".format(ft,run_no,NPARTICLES),theta)
    np.save("results_FA_ptheta_{}_{}_{}_0b.npy".format(ft,run_no,NPARTICLES),prop_theta)
    np.save("results_FA_ar_{}_{}_{}_0b.npy".format(ft,run_no,NPARTICLES),ar)
    ...
    if False:
        for i in range(100):
            ...
            with open("results_pyMC_NUTS_RJMCMC_FA_N{}_run{}_{}.txt".format(NPARTICLES,run_no,ft), "a") as file_object:
